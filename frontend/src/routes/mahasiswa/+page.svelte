...
<div class="layout-container">
  <div class="bg-transparent backdrop-blur-sm border-zinc-200/50 dark:border-zinc-800/50 flex-shrink-0 relative">
    ...
    >
      <ChevronLeft class="h-4 w-4 text-zinc-900 dark:text-zinc-50" />
    </Button>
        ...
        {#each menus as menu}
          ...
          <a
            ...
            style="
              ...
              color: {isActive ? (darkMode ? '#FAFAFA' : '#212121') : (darkMode ? '#FAFAFA' : '#212121')};
            "
      ...
      >
        ...
        <Input 
          type="search" 
          class="flex bg-gray-700/30 backdrop-filter backdrop-blur-md rounded-full p-1 transition-all duration-300 ease-in-out
...
:global(body, html) {
  ...
  .page-container {
    flex: 1;
    background: linear-gradient(to right, #FDE68A, #FACC15); /* light mode: teal-400 to yellow-200 */
    overflow-y: auto;
    margin-top: -4rem;
    padding-top: 4rem;
  }
  ...
  :global(.dark) .page-container {
    background: linear-gradient(to right, #B45309, #92400E); /* dark mode: teal-800 to yellow-700 */
  }
</style>